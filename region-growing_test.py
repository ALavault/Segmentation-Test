...
def regionGrowing(image, seeds, pixelThreshold, regionThreshold, labels = None, sortDistances = False, noOrphans = False):
    ...
    while toVisit != []:
        ...
        if nIter<=nSeeds:
            ...
            neighboursList=[(x,max(0,y-1)), (max(x-1,0),y), (x,min(nbCols-1,y+1)),(min(x+1,nbRows-1),y)] # L, T, R, B
            distances=[]
...
markers.astype(int)
plt.close('all')
labels = regionGrowing(im, markers, 25,350, noOrphans=False)
labels2 = regionGrowing(im, markers, 25,350,noOrphans=True)
